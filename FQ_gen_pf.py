...
def generate_and_save(sample_idx):
    seed = int(time.time() * 1e6) % (2**32 - 1) + os.getpid() + sample_idx
    np.random.seed(seed)
    random.seed(seed)
    ...
    try:
        ...
        np.save( DATAPATH + f'/yantian752_251001/H_{sample_idx}.npy', Hi)
        save_npz( DATAPATH + f'/yantian752_251001/Y_{sample_idx}', sparse_Yi)
...
if __name__ == "__main__":
    ...
    Y = load_npz( f'/data2/zyh/yantian_752fc_20250715/Y_{sample_idx}.npz')  # 直接加载第一个样本的Y矩阵
    # 直接处理稀疏矩阵，避免将其转为稠密矩阵
