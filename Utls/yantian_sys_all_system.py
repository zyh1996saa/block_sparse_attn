...
#import os
import sys
from config import WORKPATH,p_lower,p_upper,q_lower,q_upper,PfDataPath, nd_withload, nd_load_stat
cwd = WORKPATH
...
class FeederCluster:
    ...
    def scan_feasible_switch_states(self):
        ...
        must_power = {nd.bus for nd in self.nodes}
        # 回溯搜索
        solutions = []
...
def save_feasible_feeders_switch_states(pp_pf_calculator,path=cwd + "/实验/YanTian/"):
    ...
    for fc in pp_pf_calculator.feeder_clusters:
        ...
        fc_num += 1
    with open(path + "fcID2fcname.pkl", "wb") as f:  # 以二进制写模式打开
        pickle.dump(fcID2fcname, f)
    with open(path + "fcname2fcID.pkl", "wb") as f:  # 以二进制写模式打开
        pickle.dump(fcname2fcID, f)
    with open(path + "fcID2fs.pkl", "wb") as f:  # 以二进制写模式打开
        pickle.dump(fcID2fs, f)    
        
def load_feasible_feeders_switch_states(pp_pf_calculator,path=cwd + "/实验/YanTian/"):
    global fcID2fcname,fcname2fcID,fcID2fs
    
    with open(path+"fcID2fcname.pkl", "rb") as f:  # 以二进制读模式打开
        fcID2fcname = pickle.load(f)
    with open(path+"fcname2fcID.pkl", "rb") as f:  # 以二进制读模式打开
        fcname2fcID = pickle.load(f)
    with open(path+"fcID2fs.pkl", "rb") as f:  # 以二进制读模式打开
